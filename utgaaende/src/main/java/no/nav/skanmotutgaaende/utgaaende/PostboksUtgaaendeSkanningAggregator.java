package no.nav.skanmotutgaaende.utgaaende;
...
public class PostboksUtgaaendeSkanningAggregator implements AggregationStrategy {
    ...
    public Exchange aggregate(Exchange oldExchange, Exchange newExchange) {
        try {
            if (oldExchange == null) {
                final PostboksUtgaaendeEnvelope envelope = new PostboksUtgaaendeEnvelope(newExchange.getProperty(PROPERTY_FORSENDELSE_ZIPNAME, String.class), FilenameUtils.getBaseName(newExchange.getIn().getHeader(Exchange.FILE_NAME, String.class)));
    ...
    private void applyOnEnvelope(Exchange newExchange, PostboksUtgaaendeEnvelope envelope) throws IOException {
        final String extension = FilenameUtils.getExtension(newExchange.getIn().getHeader(Exchange.FILE_NAME, String.class));
